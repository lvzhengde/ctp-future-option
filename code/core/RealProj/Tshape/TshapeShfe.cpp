...
//构造函数
//--
CTshapeShfe::CTshapeShfe(string OptionId)
{
	//交易所代码必须是上海期货交易所
	TThostFtdcExchangeIDType ExchangeId = "SHFE"; 
	m_OptionId = OptionId;

	//上海期货交易所期权品种代码和标的品种代码一样（指合约代码的前一位或者两位）
...
//    无
//--
void CTshapeShfe::ConstructTshape(vector<CThostFtdcInstrumentField> InstInfoList)
{
	//可以接受的浮点数表示精度误差
	double eps = (1e-6);
	void *pNode = NULL;
	...
	for(int i = 0; i < InstInfoListSize; i++)
	{
		...
		else if(RetVal == 0)
		{
			...
			str2 = str1.substr(0, InstIdPrefixSize+4);
...
//     1: 意图交易的期权合约的标的期货合约
//--
int CTshapeShfe::ParseInstrumentId(TThostFtdcInstrumentIDType InstrumentID, size_t& YearMonth, double& StrikePrice, char& OptionTypeCode, void* &pNode)
{
	SYSTEMTIME CurTime;
	string strInstId;
	size_t InstIdSize;
	...
	if(strcmp(str1.c_str(), m_OptionId.c_str()) != 0 || InstIdSize < (InstIdPrefixSize+4))
		return -1;
	...
	OptionTypeCode = InstrumentID[InstIdPrefixSize+4];

	int StrikePriceLen = InstIdSize -(InstIdPrefixSize+5);
	if(StrikePriceLen <= 0)
		return -1;

	str1 = strInstId.substr(InstIdPrefixSize+5, StrikePriceLen);
...
//注意：上期所的保证金计算用到了Delta值，为方便起见Delta值一律取为1（因为卖出的多为实值期权）。
//参数
...
//    无
//--
void CTshapeShfe::CalcOptionMiscValues(COptionItem* pOptionItem, size_t nMonthPos)
{
	char OptionType;
	double Royalty;
	...
	//以下估算期权卖出开仓所需的保证金
	//为简单起见，上期所保证金计算公式中的Delta一律取值为1.0 (规则修改之后已经不再考虑Delta)
	double UsedRoyalty;
	double UsedOutOfTheMoney;
	double VolumeMultiple = (double)pOptionItem->m_InstInfo.VolumeMultiple;

	//期权合约价格, 采用简化处理方式，取昨结算价和成交价的最大者
	UsedRoyalty = max(pOptionItem->m_DepthMarketData.PreSettlementPrice, pOptionItem->m_DepthMarketData.LastPrice);

	//期权虚值额
	if(pOptionItem->m_InstInfo.OptionsType == THOST_FTDC_CP_CallOptions)
		UsedOutOfTheMoney = max((StrikePrice - m_pTshapeMonthList[nMonthPos]->m_UnderlyingMarketData.PreSettlementPrice), 0.0) * VolumeMultiple;
	else if(pOptionItem->m_InstInfo.OptionsType == THOST_FTDC_CP_PutOptions)
		UsedOutOfTheMoney = max((m_pTshapeMonthList[nMonthPos]->m_UnderlyingMarketData.PreSettlementPrice - StrikePrice), 0.0) * VolumeMultiple;

	//计算保证金
	double tmp1, tmp2;
	if(pOptionItem->m_InstInfo.OptionsType == THOST_FTDC_CP_CallOptions)      //卖出看涨期权
	{
		tmp1 = UsedRoyalty * VolumeMultiple + m_pTshapeMonthList[nMonthPos]->m_ShortUnderlyingMargin - UsedOutOfTheMoney * 0.5;
		tmp2 = UsedRoyalty * VolumeMultiple + m_pTshapeMonthList[nMonthPos]->m_ShortUnderlyingMargin * 0.5;
		pOptionItem->m_Margin = max(tmp1, tmp2);
	}
	else if(pOptionItem->m_InstInfo.OptionsType == THOST_FTDC_CP_PutOptions)  //卖出看跌期权
	{
		tmp1 = UsedRoyalty * VolumeMultiple + m_pTshapeMonthList[nMonthPos]->m_LongUnderlyingMargin - UsedOutOfTheMoney * 0.5;
		tmp2 = UsedRoyalty * VolumeMultiple + m_pTshapeMonthList[nMonthPos]->m_LongUnderlyingMargin * 0.5;
		pOptionItem->m_Margin = max(tmp1, tmp2);
	}
}
