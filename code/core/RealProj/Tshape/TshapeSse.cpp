...
//--
CTshapeSse::CTshapeSse(string OptionId, size_t UnderlyingAssetType)
{
	//交易所代码必须是上海证券交易所
	TThostFtdcExchangeIDType ExchangeId = "SSE"; 
	m_OptionId = OptionId;
...
//    无
//--
void CTshapeSse::ConstructTshape(vector<CThostFtdcInstrumentField> InstInfoList)
{
			...
			if(YearMonth == m_pTshapeMonthList[MonthItemPos]->m_YearMonth)
			{
				...
				for (int j = 0; j < OptionPairListSize; j++)
				{
					...
					if(FloatEqual(StrikePrice, m_pTshapeMonthList[MonthItemPos]->m_pOptionPairList[j]->m_StrikePrice, eps)
						&& InstInfoList[i].InstrumentCode[11] == m_pTshapeMonthList[MonthItemPos]->m_pOptionPairList[j]->m_AdjustFlag)
					{
				...
				if(bCreateOptionPairItem == true)
				{
					...
					pOptionPairItem->m_AdjustFlag = InstInfoList[i].InstrumentCode[11];
...
//     1: 意图交易的期权合约的标的证券
//--
int CTshapeSse::ParseInstrumentField(CThostFtdcInstrumentField InstrumentField, size_t& YearMonth, double& StrikePrice, char& OptionTypeCode)
{
...
//     1: 意图交易的期权合约的标的证券
//--
int CTshapeSse::ParseInstrumentId(TThostFtdcInstrumentIDType InstrumentID, size_t& YearMonth, double& StrikePrice, char& OptionTypeCode, void* &pNode)
{
	bool found = false;
	int RetVal = -1;
...
//    无
//--
void CTshapeSse::CalcOptionMiscValues(COptionItem* pOptionItem, size_t nMonthPos)
{
	char OptionType;
	double Royalty;
...
//    < 0: 不作任何改变
//--
int CTshapeSse::CreateOrReplaceOptionItem(CThostFtdcInstrumentField *pInstrumentField, COptionItem* pOptionItem)
{
	int ret = 0;
