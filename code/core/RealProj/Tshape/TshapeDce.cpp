...
//构造函数
//--
CTshapeDce::CTshapeDce(string OptionId)
{
	//交易所代码必须是大连商品交易所
	TThostFtdcExchangeIDType ExchangeId = "DCE"; 
	m_OptionId = OptionId;
	//大商所期权代码和标的代码一样（指合约代码的前一位或者两位）
...
//--
void CTshapeDce::ConstructTshape(vector<CThostFtdcInstrumentField> InstInfoList)
{
	//可以接受的浮点数表示精度误差
	double eps = (1e-6);
	void *pNode = NULL;
...
//     1: 意图交易的期权合约的标的期货合约
//--
int CTshapeDce::ParseInstrumentId(TThostFtdcInstrumentIDType InstrumentID, size_t& YearMonth, double& StrikePrice, char& OptionTypeCode, void* &pNode)
{
	SYSTEMTIME CurTime;
	string strInstId;
	size_t InstIdSize;
...
//    无
//--
void CTshapeDce::CalcOptionMiscValues(COptionItem* pOptionItem, size_t nMonthPos)
{
	char OptionType;
	double Royalty;
